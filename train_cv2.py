...
List_preact = [False, False, True, True]
List_dropout = [0, 0.1, 0, 0.1]
...
def train():
    ...
    scores = np.empty((len(List_preact), 5))

    for run, (preact, dropout) in enumerate(zip(List_preact, List_dropout)):
        ...
        for fold in range(5):
            ...
            # Train
            save_dir = os.path.join("checkpoint", f"cv2_run_{run}_fold_{fold}")
            ...
            wandb.init(
                ...
                name=f"cv2_run_{run}_fold_{fold}",
    ...
    np.savetxt("scores_cv2.csv", scores, delimiter=",", fmt="%.6f")
